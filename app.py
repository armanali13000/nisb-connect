...
def register_page():
    if request.method=='POST':
        ...
        if username and password and email and bio and dob and ieee_no and branch and sem:
            user_register(username,password,email,bio,dob,ieee_no,branch,sem)
            return redirect('/login')
        else:
            return render_template('register.html',err="Please Fill all stuff")
    else:
...
def login_page():
    if request.method=='POST':
        ...
        if result:
            ...
            return render_template('login.html',err='You are doing it wrong pal.')
...
def view_groups_stream(group_id):
    g_info = get_group_info(group_id)
    if g_info:
        return render_template('group_stream.html',group=g_info,allposts=group_post_list(group_id))
    else:
