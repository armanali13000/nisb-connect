...
def user_register(username,password,email,bio,dob,ieee_no,branch,sem):
    ...
    executeDB(c,"insert into members(username,password,email,bio,dob,ieee_no,branch,sem) \
    values(%s,%s,%s,%s,%s,%s,%s,%s)",(username,password,email,bio,dob,ieee_no,branch,sem))
    disconnectDB(c)
    return True
...
def add_post(user_id,post_data,group):
    c = connectDB()
    executeDB(c,"insert into posts(user_id,wgroup,article,date_time) values(%s,%s,%s,now())",(user_id,group,post_data))
    disconnectDB(c)
    return True
...
def send_message(from_id,to_id,message):
    c = connectDB()
    executeDB(c,"insert into messages(from_id,to_id,message_data,date_time) values(%s,%s,%s,now())",(from_id,to_id,message))
    disconnectDB(c)
    return True
...
def create_group(name,desc,user_id):
    c = connectDB()
    rowid = executeDB(c,"insert into groups(name,description,members) values(%s,%s,%s) RETURNING group_id",(name,desc,str(user_id)))
    rid = rowid.fetchone()[0]
    disconnectDB(c)
    return str(rid)
